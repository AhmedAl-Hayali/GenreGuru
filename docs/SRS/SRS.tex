...
\begin{itemize}
  ...
  to be used. This is important because depending on the the source of the algorithm training data or the inputted tracks into the service
  might have strict copyright laws \& rules associated with them, thus we need to consider whether those rules are inherited with the 
  generated snippet. This could be for example, whether the end user is allowed to use the generated snippet commercially. This means 
  that creative professionals ability to actually use this portion of the service could be greatly limited depending on what tracks they
  are inputting. 
  ...
  machine learning algorithms on multiple, different datasets, or we need find a way to integrate more niche genres of music into the 
  dataset we are already using for training. 
